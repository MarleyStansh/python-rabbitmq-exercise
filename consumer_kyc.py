...
def process_new_message(
    ...
    log.warning("[ ] Start checking user email for security %r.", body)
    ...
    log.warning(
        "[X] Finished checking user mail in %.2fs, message %r is OK.",
        end_time - start_time,
        body,
    )
...
def main():
    ...
    with EmailUpdatesRabbit() as rabbit:
        rabbit.consume_messages(
            ...
            queue_name=config.RMQ_QUEUE_NAME_KYC_EMAIL_UPDATES,
        )
